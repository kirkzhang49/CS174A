...
inherit(sphere, shape);
	...
	sphere.prototype.populate = function ( recipient, points_transform, max_subdivisions ) 
		...
		function ownshape1( points_transform ) 
 { 
	 shape.call(this); 
	 if( !arguments.length) return; // Pass no arguments if you just want to statically call functions up the inheritance chain, and / or populate() 
 ...
 inherit(ownshape1, shape); 
 
 ownshape1.prototype.populate = function( recipient, points_transform ) 
 { 
	 
	  recipient.vertices = [        vec3(0,0,0), vec3(0.5,1,0), vec3(1,0,0),
	                                vec3(0.5,-1,0),vec3(-0.5,-1,0),vec3(1.5,-1,0),
									         vec3(0,-2,0),vec3(1,-2,0),vec3(-0.5,1,0), 
											 vec3(1.5,1,0),vec3(0.5,2,0)]; 
	 recipient.normals = [  vec3(0,0,1), vec3(0,0,1), vec3(0,0,1)
	                       ,vec3(0,0,1), vec3(0,0,1), vec3(0,0,1),
						    vec3(0,0,1),vec3(0,0,1),vec3(0,0,1),vec3(0,0,1),
							vec3(0,0,1)]; 
	 recipient.texture_coords = [    vec2(0,0), vec2(0.5,1), vec2(1,0),
	                                    vec2(0.5,-1),vec2(-0.5,-1),vec2(1.5,-1),
										vec2(0,-2),vec2(1,-2),vec2(-0.5,1),vec2(1.5,1),
										vec2(0.5,2)]; 
	 recipient.indices = [      0, 1, 2 ,0,2,3,0,3,4,2,3,5,4,3,6,3,5,7,0,1,8,1,2,9,8,9,10]; 
 };
function ownshape2( points_transform ) 
 { 
	 shape.call(this); 
	 if( !arguments.length) return; // Pass no arguments if you just want to statically call functions up the inheritance chain, and / or populate() 
	 this.populate( this, points_transform ); 
 ...
 inherit(ownshape2, shape); 
 
 ownshape2.prototype.populate = function( recipient, points_transform ) 
 { 
	 
	  recipient.vertices = [   vec3(0,0,0),vec3(0,-4,-2),vec3(-2,-4,2),vec3(2,-4,2),vec3(0,4,-2),vec3(-2,4,2),vec3(2,4,2),vec3(0,0,0) ,vec3(0,0,0),vec3(0,0,0),vec3(0,0,0),vec3(0,0,0)    ]; 
	 recipient.normals = [vec3(1,0,0), vec3(0,1,0), vec3(0,0,1),vec3(0,1,0),vec3(0,0,1), vec3(0,1,0),vec3(0,0,1),vec3(1,0,0),vec3(1,0,0),vec3(1,0,0),vec3(1,0,0),vec3(1,0,0) ]; 
	 recipient.texture_coords = [   vec2(0,0),vec2(0,-4),vec2(-2,-4),vec2(2,-4),vec2(0,4),vec2(-2,4),vec2(2,4),vec2(0,0),vec2(0,0),vec2(0,0),vec2(0,0),vec2(0,0)]; 
	 recipient.indices = [ 0,1,2,7,1,3,8,2,3 ,9,4,5,10,4,6,11,5,6,1,2,3,4,5,6    ]; 
 };
